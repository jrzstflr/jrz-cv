...
const Services = () => {
    ...
    const [modalLink, setModalLink] = useState('');  // Added to store the link
    ...
    const servicesData = [
        ...
        {
            title: "N/A",
            description: "Lorem ipsum dolor sit amet, consectetur adipisicing elit. Consequatur corrupti possimus quisquam sequi quibusdam deserunt praesentium architecto facilis, enim voluptatem, laborum cumque repudiandae quia? Optio eos nihil reiciendis laborum, nulla asperiores at.",
            image: jrzlogo,
        },
        {
            title: "N/A",
    ...
    const handleImageClick = (image, title, link) => {
        setModalImage(image);
        setModalTitle(title);
        setModalLink(link);  // Set the link
        setIsModalOpen(true);
    };
    ...
    return (
        <div id="services" className='text-white py-16'>
            <motion.div
                ...
                <h2 className='text-3xl md:text-4xl font-bold mb-8 border-b-2 border-purple-500 inline-block'>Certificates</h2>
                <p className='mb-12 text-gray-400'>This section highlights the various professional certifications I have earned, demonstrating my commitment to continuous learning and growth in Network Troubleshooting, Contact Center, Integrations, and Web Development. Each certification reflects my dedication to mastering essential skills, staying current with industry trends, and enhancing my capabilities to provide value in my professional endeavors.</p>
                ...
                <Slider {...settings}>
                    {servicesData.map((service, index) => (
                        <motion.div
                            ...
                            key={index}
                            className='bg-[#1d1a2b] rounded-lg p-6 text-center hover:shadow-lg hover:shadow-purple-500 transition-shadow duration-300'>
                            <img 
                                src={service.image} 
                                alt={`${service.title} Certificate`} 
                                className='text-purple-500 text-4xl sm:text-5xl lg:text-6xl mb-4 mx-auto cursor-pointer'
                                onClick={() => handleImageClick(service.image, service.title, service.link)} // Handle image click
                            />
            ...
            <Modal 
                ...
                imageSrc={modalImage} 
                title={modalTitle}
                link={modalLink} // Pass the link to the Modal component
            />
        </div>
    );
};
